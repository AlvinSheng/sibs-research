...
#' @param params parameters scale and sigma for the baseline hazard
...
simulate_dist <- function(dist, x, fcts_select, params, FUN, ...) {
  ...
  for (i in 1:n) {
    ...
    time[i] <- exp(params$scale + x_select[i,] %*% betas + params$sigma * z)
...
I use arbitrary Weibull parameters to make the survival times and censoring rate be similar to those of the ACTG-175 data set.
...
I use arbitrary log-normal parameters to make the survival times and censoring rate be similar to those of the ACTG-175 data set. Of note, the scale is the same, i.e. equal to -11. 
...
tic()
sims <- simulate_regression(N = 100, p = 16, "weibull", x, 
...
tic()
sims <- simulate_regression(N = 100, p = 16, "lnorm", x,  
                            fcts_select, params = list(scale = -11, sigma = sigma),
                            FUN = rexp, rate = 1.3)
