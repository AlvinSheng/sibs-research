...
simulate_regression <- function(N, p, simulate_fun = simulate_weibull, ...) {
  ...
  for (i in 1:N) {
    ...
    # only storing the coefficient values
...
Testing simulate_regression function

```{r}
set.seed(636)

sr_test <- simulate_regression(N = 500, p = 16, simulate_fun = simulate_weibull, x, fcts_select, 
                    params = list(shape = parm_res$pars[2],
                                  scale = parm_res$pars[1]), 
                    FUN = rexp, rate = 1.3)
