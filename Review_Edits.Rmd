...
#' @param params parameters shape and rate for the baseline Weibull distribution, by default the exponential distribution with rate = 1
...
#' @return 
#' @export
simulate_weibull <- function(x, fcts_select, params = list(shape = 1, rate = 1), FUN, ...) {
  ...
  u <- runif(n)
  time <- (-log(u) / (params$rate * exp(x_select %*% betas)))^(1 / params$shape)
